...
def show_recommend():
    ...
    @st.cache_resource
    def get_recommender_cache(_version="v20260706_16"):
    ...
    with st.sidebar:
        ...
        if selected_shed:
            ...
            # 切换棚舍时自动同步动物种类
            if st.session_state.get("app_prev_shed_id") != selected_shed.id:
                st.session_state["app_animal_type"] = animal_type_default
                st.session_state["app_prev_shed_id"] = selected_shed.id
        else:
        ...
        animal_type = st.selectbox(
            "动物种类",
            animal_type_options,
            index=animal_type_options.index(st.session_state.get("app_animal_type", animal_type_default)),
            help="选择养殖的动物种类",
            key="app_animal_type"
        )
        ...
        if selected_shed:
            ...
            if medication_history:
                st.info(f"当前批次已记录 {len(medication_history)} 条化药历史用药，系统推荐时将自动排除这些药物及同类易产生交叉耐药的药物")
                ...
                st.info("暂无化药历史用药记录。如果是刚开始使用软件，请手动添加之前使用过的化药。")
        ...
        if not symptom:
            ...
            with st.spinner("正在分析病情并推荐最佳用药方案..."):
                try:
                    ...
                    recommender = get_recommender_cache("v20260706_16")
