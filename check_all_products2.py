...
for product in products:
    ...
    if is_banned:
        ...
        for ingredient, info in banned_reasons.items():
            if ingredient in name or (matched_banned_name and ingredient in matched_banned_name):
        ...
        matched_banned.append({
            ...
            'matched_banned_drug': matched_banned_name,
...
result = {
    ...
    'non_banned_count': len(not_matched),
...
with open('banned_drugs_full_report.json', 'w', encoding='utf-8') as f:
    json.dump(result, f, ensure_ascii=False, indent=2)

print(f"Total products: {len(products)}")
print(f"Banned drugs: {len(matched_banned)}")
print(f"Non-banned products: {len([p for p in not_matched if p['product_name'] != '/' and p['product_name'] != 'nan' and p['product_name']])}")
print("\nResults saved to banned_drugs_full_report.json")
